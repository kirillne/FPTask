...
(defn add-rating [request user-id comment-id value post-id]
	(let [
		exists (get-comment-existing comment-repository user-id comment-id)
		bool-value (if (= value "1") (boolean true) (boolean false) )] 
		(if (nil? exists) 
			(add-comment-rating comment-repository user-id comment-id bool-value) 
			(update-comment-rating comment-repository user-id comment-id bool-value))
